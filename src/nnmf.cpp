                         ...
                         const double inner_rel_tol,  unsigned int trace)
{
  ...
  for(; i < max_iter && std::abs(rel_err) > rel_tol; i++)
  {
    ...
    // update W
    total_raw_iter += update_with_marker(W, H, A.t(), Wm,true,  inner_max_iter, inner_rel_tol, n_threads);
    // update H
    total_raw_iter += update_with_marker(H, W, A, Hm,false,  inner_max_iter, inner_rel_tol, n_threads);
